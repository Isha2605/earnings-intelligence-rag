...
def _confidence(nodes):
    ...
    avg_score = max(0.0, min(1.0, sum(scores) / len(scores))) if scores else 0.5
    ...
    unique_years  = len({y for y in years if y is not None})
    cross_coverage = unique_years / 3.0

    score = max(0.0, min(1.0, 0.5 * avg_score + 0.3 * source_agreement + 0.2 * cross_coverage))
